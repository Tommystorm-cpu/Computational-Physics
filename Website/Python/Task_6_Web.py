...
def task_6(system, planet1, planet2):
    fig = plt.figure()
    ax = fig.add_subplot()
    ...
    two_planets = [planet_system[planet1], planet_system[planet2]]

    num_of_orbits = 10
    if (float(two_planets[1][6]) > float(two_planets[0][6])):
        total_time = num_of_orbits * float(two_planets[1][6])
    else:
        total_time = num_of_orbits * float(two_planets[0][6])
